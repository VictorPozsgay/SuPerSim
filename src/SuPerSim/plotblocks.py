...
def plot_hor(path_horizon, list_fig_names, list_figs, show_plots):
...
def plot_hist(site, path_pickle,path_thaw_depth, no_weight, show_glaciers, list_fig_names, list_figs, show_plots, query, query_suffix):
    if show_plots:
        print('\n---------------------------------------------------------------------------------------------\n')
...
def plot_yearly_stats_sims(query, query_suffix, time_ground, temp_ground, snow_height, swe,
    ...
    if query is not None:
        ...
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, temp_ground, list_valid_sim_query, 'GST', year_bkg_end, year_trans_end, show_plots))
        list_fig_names.append('GST_yearly_mean'+query_suffix)
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, temp_ground, list_valid_sim_query, 'GST', year_bkg_end, year_trans_end, show_plots, plot_quantiles=False))
        for k,v in idxs_depths.items():
            list_fig_names.append(f'{k}m_grdtemp_yearly_quantiles'+query_suffix)
            list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, temp_ground, list_valid_sim_query, f'{k}m ground temperature', year_bkg_end, year_trans_end, show_plots, idx_depth=v))
            list_fig_names.append(f'{k}m_grdtemp_yearly_mean'+query_suffix)
            list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, temp_ground, list_valid_sim_query, f'{k}m ground temperature', year_bkg_end, year_trans_end, show_plots, plot_quantiles=False, idx_depth=v))
        list_fig_names.append('Snow_yearly_quantiles'+query_suffix)
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, snow_height, list_valid_sim_query, 'Snow depth', year_bkg_end, year_trans_end, show_plots))
        list_fig_names.append('Snow_yearly_mean'+query_suffix)
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, snow_height, list_valid_sim_query, 'Snow depth', year_bkg_end, year_trans_end, show_plots, plot_quantiles=False))
        list_fig_names.append('SWE_yearly_quantiles'+query_suffix)
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, swe, list_valid_sim_query, 'SWE', year_bkg_end, year_trans_end, show_plots))
        list_fig_names.append('SWE_yearly_mean'+query_suffix)
        list_figs.append(plot_yearly_quantiles_sim_from_inputs(time_ground, swe, list_valid_sim_query, 'SWE', year_bkg_end, year_trans_end, show_plots, plot_quantiles=False))
...
def plot_thaw_depth(query, query_suffix, time_ground, thaw_depth, thaw_depth_query,
    ...
    if query is not None:
        ...
        list_figs.append(plot_yearly_max_thaw_depth_from_inputs(time_ground, thaw_depth_query, list_depths, df_query, show_plots))
...
def plot_2metrics_seasonal(query, query_suffix, time_ground, temp_ground, snow_height,
    ...
    if query is not None:
        ...
        list_figs.append(plot_sanity_two_variables_one_year_quantiles_from_inputs(time_ground, [temp_ground, snow_height], [list_valid_sim_query, list_valid_sim_query], ['GST', 'Snow depth'], show_plots))
...
def plot_1metric_seasonal(query, query_suffix, time_ground, time_bkg_ground, time_trans_ground,
    ...
    if custom_years is not None:
        ...
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground]*(len(custom_years)+1), [list_valid_sim]*(len(custom_years)+1),
                                                                                'GST', ['Background']+[f'Year {k}' for k in custom_years], [time_bkg_ground]+[list_tokens_year(time_ground, k, k+1)[2] for k in custom_years], show_plots))
    if query is not None:
        list_fig_names.append('GST_1year_bkg_v_transient'+query_suffix)
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground, temp_ground], [list_valid_sim_query, list_valid_sim_query], 'GST', ['Background', 'Transient'], [time_bkg_ground, time_trans_ground], show_plots))
        if show_excep_years:
    ...
    if show_decades:
        ...
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground]*(len(list_decs)+2), [list_valid_sim]*(len(list_decs)+2),
                                                                                'GST', ['Background', 'Transient']+[f'Decade {dec}-{dec+9}' for dec in list_decs], [time_bkg_ground, time_trans_ground]+[list_tokens_year(time_ground, dec, dec+10)[2] for dec in list_decs], show_plots))
    ...
    for k,v in idxs_depths.items():
        ...
        if custom_years is not None:
            ...
            list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground]*(len(custom_years)+1), [list_valid_sim]*(len(custom_years)+1),
                                                                                    f'{k}m ground temperature', ['Background']+[f'Year {k}' for k in custom_years], [time_bkg_ground]+[list_tokens_year(time_ground, k, k+1)[2] for k in custom_years], show_plots, idx_depth=v))
        if query is not None:
            list_fig_names.append(f'{k}m_grdtemp_1year_bkg_v_transient'+query_suffix)
            list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground, temp_ground], [list_valid_sim_query, list_valid_sim_query], f'{k}m ground temperature', ['Background', 'Transient'], [time_bkg_ground, time_trans_ground], show_plots, idx_depth=v))
            if show_excep_years:
        ...
        if show_decades:
            ...
            list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [temp_ground]*(len(list_decs)+2), [list_valid_sim]*(len(list_decs)+2),
                                                                                    f'{k}m ground temperature', ['Background', 'Transient']+[f'Decade {dec}-{dec+9}' for dec in list_decs], [time_bkg_ground, time_trans_ground]+[list_tokens_year(time_ground, dec, dec+10)[2] for dec in list_decs], show_plots, idx_depth=v))
    ...
    if custom_years is not None:
        ...
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [snow_height]*(len(custom_years)+1), [list_valid_sim]*(len(custom_years)+1),
                                                                                'Snow depth', ['Background']+[f'Year {k}' for k in custom_years], [time_bkg_ground]+[list_tokens_year(time_ground, k, k+1)[2] for k in custom_years], show_plots))
    if show_decades:
        list_decs = []
        for dec in range(int((year_trans_end-year_bkg_end-1)/10)):
            list_decs.append(year_bkg_end+1+dec*10)
        ...
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [snow_height]*(len(list_decs)+2), [list_valid_sim]*(len(list_decs)+2),
                                                                                'Snow depth', ['Background', 'Transient']+[f'Decade {dec}-{dec+9}' for dec in list_decs], [time_bkg_ground, time_trans_ground]+[list_tokens_year(time_ground, dec, dec+10)[2] for dec in list_decs], show_plots))
    if query is not None:
        list_fig_names.append('Snow_1year_bkg_v_transient'+query_suffix)
        list_figs.append(plot_sanity_one_year_quantiles_two_periods_from_inputs(time_ground, [snow_height, snow_height], [list_valid_sim_query, list_valid_sim_query], 'Snow depth', ['Background', 'Transient'], [time_bkg_ground, time_trans_ground], show_plots))
        if show_excep_years:
...
def plot_quantiles(site, path_pickle, show_plots, list_fig_names, list_figs):
    if show_plots:
        print('\n---------------------------------------------------------------------------------------------\n')
...
def plot_meltout(site, path_pickle, show_plots, list_fig_names, list_figs):
    if show_plots:
        print('\n---------------------------------------------------------------------------------------------\n')
...
def plot_GST_evol(site, path_pickle, show_plots, list_fig_names, list_figs):
    if show_plots:
        print('\n---------------------------------------------------------------------------------------------\n')
        print('Plot of mean GST evolution vs background GST, fit, and binning per 10% quantiles')
    list_fig_names.append('GST_evol_v_bkg')
    list_figs.append(plot_GST_bkg_vs_evol_quantile_bins_fit_single_site_from_inputs(site, path_pickle, show_plots, query=None))
...
def plot_parity(site, path_pickle, show_plots, list_fig_names, list_figs, all_data=False, diff_forcings=True):
    if show_plots:
        print('\n---------------------------------------------------------------------------------------------\n')
    ...
    fig_model, _, _, _, _, _, _ = fit_stat_model_GST_from_inputs(site, path_pickle, show_plots, all_data, diff_forcings)

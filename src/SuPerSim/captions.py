...
capt_norm_dev = {
    ...
    'normdev_bar': 'Plot of the normalized deviation (or standardized anomaly, defined to be the deviation away from the mean in units of standard deviation) of mean annual values with respect to background values for the total water production, air temperature, and ground temperatures (surface, 1m ,5m, and 10m). By definition, the sum of all deviations within the background cancel out. On top of this, for each year in the transient area, its mean annual value is compared to the ones of all previous years and a percentile is computed. A marker indicates whether it is a new maximum, or a particularly high value relative to previous years.'
}
